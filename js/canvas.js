...
let bucketButton = document.querySelector('#bucketBtn');
    ...
    if (mouseDown && mouseInBounds())
    {
        ...
        if (currentTool === ToolType.PEN) {
            if (toolSize === 2) {
                ...
                pixelData[pixelX][pixelY-1] = penColor;
                pixelData[pixelX-1][pixelY-1] = penColor;
                pixelData[pixelX-1][pixelY] = penColor;
            }
            else {
                pixelData[pixelX][pixelY] = penColor;
            }
        ...
        else if (currentTool === ToolType.BUCKET) {
            colorToReplace = pixelData[pixelX][pixelY];
            bucketFill(colorToReplace, pixelX, pixelY);
        }
        drawSprite();
    }
}

// Fill all "recursively adjacent" pixels (diagonals dont count) that are the same color (including transparent) with the new color
function bucketFill(colorToReplace, pixelX, pixelY) {
    if (pixelX < 0 || pixelX > spriteSize-1 || pixelY < 0 || pixelY > spriteSize-1
        || pixelData[pixelX][pixelY] != colorToReplace) {
        return;
    }
    pixelData[pixelX][pixelY] = penColor;
    bucketFill(colorToReplace, pixelX+1, pixelY)
    bucketFill(colorToReplace, pixelX-1, pixelY)
    bucketFill(colorToReplace, pixelX, pixelY+1)
    bucketFill(colorToReplace, pixelX, pixelY-1)
}
